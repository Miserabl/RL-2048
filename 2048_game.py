...
def draw():
    ...
    screen.blit(
        font2.render("Score: " + str(score), True, (50, 50, 50)),
        (start_L - 25, start_T - 150),
    )
    ...
    for i in range(16):
        ...
        col = i % 4
        rect1 = pygame.Rect(
            start_L + (size_square + size_line) * col,
            start_T + (size_square + size_line) * row,
            size_square,
            size_square,
        )
    ...
    for i in tiles:
        ...
        screen.blit(
            font3.render(str(tiles[i].value), True, (50, 50, 50)),
            (tiles[i].top + size_square / 3 - 15, tiles[i].left + size_square / 3 - 10),
        )
    ...
    for i in range(15, 3, -1):
        if i in tiles and i - 4 not in tiles:
            tiles[i - 4] = tile(
                ...
                size_square,
                tiles[i].value,
                start_L + (size_square + size_line) * ((i - 4) % 4),
                start_T + (size_square + size_line) * ((i - 4) // 4),
            )
            del tiles[i]
            var = True
    ...
    for i in range(16):
        if i in tiles and i + 1 not in tiles and (i + 1) % 4 != 0:
            tiles[i + 1] = tile(
                ...
                size_square,
                tiles[i].value,
                start_L + (size_square + size_line) * ((i + 1) % 4),
                start_T + (size_square + size_line) * ((i + 1) // 4),
            )
            del tiles[i]
            var = True
    for i in range(16):
...
tiles[num1] = tile(
    ...
    size_square,
    2,
    start_L + (size_square + size_line) * (num1 % 4),
    start_T + (size_square + size_line) * (num1 // 4),
)
tiles[num2] = tile(
    num2,
    size_square,
    val2,
    start_L + (size_square + size_line) * (num2 % 4),
    start_T + (size_square + size_line) * (num2 // 4),
)
...
def endgame():
    ...
    screen.blit(
        font.render("Game Over", True, (50, 50, 50)), (start_L + 50, start_T + 200)
    )
